...
class Dictionary(object):
    ...
    def max_vocab(self, max_vocab):
        ...
        self.check_valid()
        tf.logging.info("Maximum vocabulary size: %i. Dictionary size: %i -> %i (removed %i words)."
    ...
    def min_count(self, min_count):
        ...
        self.counts = {k: v for k, v in self.counts.items() if k in self.word2id}
        self.check_valid()
        tf.logging.info("Minimum frequency count: %i. Dictionary size: %i -> %i (removed %i words)."
    ...
    def read_vocab(vocab_path):
        ...
        tf.logging.info("Read %i words from the vocabulary file." % len(dico))
        if skipped > 0:
            tf.logging.warning("Skipped %i empty lines!" % skipped)
        return dico

    @staticmethod
    def index_data(seg_word_list, dico):
        indexed = [1]
        for w in seg_word_list:
            ...
            if 0 <= word_id < 4 + SPECIAL_WORDS and word_id != 3:
                tf.logging.warning('Found unexpected special word "%s" (%i)!!' % (w, word_id))
                continue
            assert word_id >= 0
