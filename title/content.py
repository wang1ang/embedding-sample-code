...
def output_layer(x, dim, scope, activation):
    return tf.layers.dense(x, units=dim, activation=activation, kernel_initializer=tf.initializers.glorot_normal(), kernel_regularizer=tf.contrib.layers.l2_regularizer(0.00004), name=scope) #tf.keras.initializers.he_normal()
